...
export function BatchTimeDeployment({ batchId, type, timeType, time_end_date, time_end_time, time_start_date, time_start_time }: Props) {
	return (
		<form
			hx-swap="none"
		...
		>
			...
			<div className="flex items-center justify-between">
				...
				<div className="flex items-center gap-3">
					{ match([timeType, type])
						.with(["time1", "ascent"], () => (
							<>
								<input disabled value={ time_start_date } required name="time_start_date" type="date" class="input w-[200px]" />
								<input value={ time_start_date } required name="time_start_date" type="hidden" />
							</>
						)) 
						.with(["time2", "ascent"], () => (
							<>
			...
			<div className="flex items-center justify-between mb-3">
				...
				<div className="flex items-center gap-3">
					{ match([timeType, type])
						.with(["time1", "ascent"], () => (
							<>
								<input disabled value={ time_end_date } required name="time_end_date" type="date" class="input w-[200px]" />
								<input value={ time_end_date } required name="time_end_date" type="hidden" />
							</>
						)) 
						.with(["time2", "ascent"], () => (
							<>
			...
			<div x-show="editable" className="flex justify-between w-full gap-2">
				<button x-on:click="editable = false" type="reset" className="btn w-[50%]">Cancel</button>
