...
export class BatchAssessment {
	...
	static create(uuid: string, token: string, title: string, type: string, organization_name: string, batch_time_start?: string) {
		return new BatchAssessment(uuid, token, title, BatchTypeMapping.fromString(type), organization_name, batch_time_start)
	}
